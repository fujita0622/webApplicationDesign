<?php
  ...
  try {
    ...
    while ($line = fgets($fp)) {
      // 取得した行($line)と削除したいユーザ情報($delete_user)が等しい場合
      // ※ trim関数で先頭と末尾にスペースがあったら削除
      if(trim($line) == trim($delete_user)) {
        // ループをスキップする
        continue;
      }
      // ユーザ情報の上書きデータ($overwrite_date)の配列に 取得した行($line)を入れる
      $overwrite_date[] = $line;
    }
...
<head>
  <meta charset="UTF-8">
  <title>ユーザ情報削除結果画面</title>
  <link rel="stylesheet" type="text/css" href="/css/style.css">
</head>
<body>
  <p>氏名（漢字）：<?php echo $delete_user_items[0]; ?></p>
  <p>メールアドレス：<?php echo $delete_user_items[4]; ?></p>
  <p>電話番号：<?php echo $delete_user_items[3]; ?></p>
  <p>以下の内容で削除しました。</p>
  <input type="button" onClick="location.href='delete.php'" value="戻る">
</body>
</html>